...
name: industry-lens
description: >
  行业关键词百科页面生成器（v1.3）。输入一个行业名（如"智能驾驶"），直接输出一个完整的单页 HTML：
  100 个关键词卡片（专业释义 + Wiki/Google + 行业数据 + 大白话 四区块）
  + 3 家标杆公司深度拆解（生态位 + 护城河 + 生命周期 + 收入结构）
  + 产业链上中下游 3 层 12-14 细分研报风可视化（毛利/议价徽章 + 真实公司池 + 跨层重复零容忍）。
  适用于：快速了解陌生行业、投研辅助、行业 Wiki 建设、给非专业读者科普。
  已发布在线示例：https://znzjluo1008-netizen.github.io/industry-lens-skill/
