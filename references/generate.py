...
def build_html(industry_name: str, keywords: list, companies: list, industry_desc: str = '') -> str:
    """拼装 HTML"""
...
def generate_industry_page(industry_name: str, output_path: str = None, preloaded_source: str = None):
    """
    主入口：生成完整行业百科 HTML
            ...
            if m:
                ...
                for ind_id, payload in data.items():
                    if payload.get('industry', {}).get('name') == industry_name:
                        ...
                        print(f'  ✨ 从 preloaded 中找到「{industry_name}」: {len(keywords)} 词 + {len(companies)} 公司')
                        break
    ...
    # Build HTML
    html = build_html(industry_name, keywords, companies, desc)
...
if __name__ == '__main__':
    ...
    generate_industry_page(args.industry, args.output, args.preloaded)
